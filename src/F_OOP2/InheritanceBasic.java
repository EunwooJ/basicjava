...
public class InheritanceBasic {
	public static void main(String[] args) {
		...
		4. Object클래스 (모든 클래스의 조상)
		  ...
		  - toString()나 equal()와 같은 메서드를 따로 정의하지 않고 사용할 수 있는것은 
		    Object 클래스에 정의된 것들이기 때문이다. 
		
		5. 오버라이드(override)
		  - 조상클래스로부터 상속받은 메서드의 내용을 변경(재정의)하는 것.
		  - 오버라이드 되기 위한 조건 => 선언부가 일치 해야 한다.
		    : 자손클래스에서 오버라이드 하는 메서드는 조상클래스의 메서드의 명과 동일해야 한다.
		    : 매개변수의 타입과 개수가 같아야 한다. //여기까지 선언부니까 같아야 함
		    : 리턴타입이 같아야 한다. //여기서부터 선언부니까 같아야 함
		    : 접근제어자를 조상 클래스보다 좁은 범위로 설정할 수 없다.
		    : 예외는 조상 클래스의 메서드보다 많이 선언할 수 없다. //같거나 작아야 한다
		    
		6. super.
		  - 조상 클래스의 멤버와 자손클래스의 멤버가 중복정의되어 있을때 서로 구분하기 위해 사용
		  - 조상멤버와 자신의 멤버를 구별하는데 사용한다는 것을 제외하고는 this.과 근본적으로 동일
		  - static 메서드에서 사용할 수 있다.
		...
		7. super()
		  ...
		  - 생성자의 첫 줄에는 조상 클래스의 생성자를 호출해야 한다.
		      자손클래스의 멤버가 조상클래스의 멤버를 사용할 수 있음으로 조상 멤버들이 먼저 초기화 되어야 한다.   
	    ...
	    8. 제어자
	      ...
	      - final(마지막의, 변경할 수 없는)
	        ...
	        : 메서드에 사용 시 오버라이드 불가하다.
	        : 클래스에 사용 시 상속할 수 없는 클래스가 된다.
	      - abstract(추상의, 미완성된)
	        : 메서드와 클래스에만 사용가능
	        : 메서드에 사용 시 구현부는 작성하지 않는다.
	          void method();
	              
	    9. 접근제어자
	      - private : 같은 클래스 내에서 접근가능 (변수, 메서드, 클래스사용x)
	      - default : 같은 패키지내에서 접근가능, (클래스o)
	      - protected : 같은 패키지 + 다른 패키지의 자손클래스에서 접근가능
	      - public  : 접근제한 없다.(클래스o) 
	      - 클래스에는 dafault, public에서만 사용 가능
	    ...
	    13. instanceof
	      ...
	      - 클래스메서드와 클래스변수는 참조변수 타입에 영향을 받는다.
	    ...
	    16. 추상클래스(abstract class)
	       ...
	       - 추상메서드
	         ...
	         abstract 리턴타입 메서드명();
	       - 추상클래스로부터 상속받는 자손클래스는 오버라이드를 통해 조상인 클래스의 추상메서드를 구현해 주면된다.
	    ...
	    17. 인터페이스(interface)
	       ...
	       - 인터페이스의 상속
	         : 인터페이스는 인터페이스만 상속 받을 수 있다. 	
	      	 ...
	      	 : 인터페이스는 다중상속 가능하다. 구현이 안되어 있어서 어떤 메서드를 쓰던지 상관없으므로	
	    ...
	    18. 내부클래스(inner class)
	       ...
	       - 스태틱클래스
	         : 외부클래스의 클래스멤버들과 관련된 작업을 수행, 클래스 메서드 관련된 작업을 수행한다.
	       - 지역클래스
